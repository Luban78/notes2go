...
(() => {
  ...
  function vytvor() {
    ...
    panel.innerHTML=`<header class="ln-nvt-head"><div class="ln-nvt-title"><strong>🖥️ Plán PC – živé ladění</strong><small>Kalendář + Připomínky</small></div><button id="dpv-min" class="ln-nvt-icon" type="button">—</button><button id="dpv-close" class="ln-nvt-icon" type="button">×</button></header><div class="ln-nvt-body">
      ...
      ${radek("dpv-rowpad","Svislé odsazení řádku",2,16)}
      ${radek("dpv-rowmin","Min. výška řádku",38,76)}
      ...
      ${radek("dpv-sfont","Velikost textu",12,22)}
      <div class="ln-nvt-section-title">PŘIPOMÍNKY – ČASOVÁ OSA</div>
      ${radek("dpv-rfilter","Šířka panelu filtrů",190,320)}
      ${radek("dpv-rheight","Min. výška řádku",44,82)}
      ${radek("dpv-rfont","Velikost textu úkolu",13,22)}
      ${radek("dpv-rtime","Velikost času",13,22)}
      ${radek("dpv-rgroup","Velikost nadpisu dne",14,24)}
      ${radek("dpv-rgap","Mezera mezi dny",8,30)}
      ${radek("dpv-rradius","Zaoblení skupiny",0,22)}
    ...
    ["screen","share","grid","dayh","daygap","dayrad","dayfont","apad","arad","afont","rowpad","rowmin","rowgap","time","sh","sr","sg","sfont","rfilter","rheight","rfont","rtime","rgroup","rgap","rradius"].forEach(k=>refs[k]=panel.querySelector(`#dpv-${k}`));
  ...
  function refresh(){ if(!panel)return; const s=api()?.ziskejStav?.(), o=api()?.ziskejVychozi?.(); if(!s||!o)return;
    setRange(refs.screen,s.kalendar.sirkaScreenu,o.kalendar.sirkaScreenu); setRange(refs.share,s.kalendar.podilKalendare,o.kalendar.podilKalendare,"%"); setRange(refs.grid,s.kalendar.sirkaMrizky,o.kalendar.sirkaMrizky); setRange(refs.dayh,s.kalendar.vyskaDne,o.kalendar.vyskaDne); setRange(refs.daygap,s.kalendar.mezeraDni,o.kalendar.mezeraDni); setRange(refs.dayrad,s.kalendar.radiusDne,o.kalendar.radiusDne); setRange(refs.dayfont,s.kalendar.pismoDne,o.kalendar.pismoDne); setRange(refs.apad,s.agenda.padding,o.agenda.padding); setRange(refs.arad,s.agenda.radius,o.agenda.radius); setRange(refs.afont,s.agenda.pismo,o.agenda.pismo); setRange(refs.rowpad,s.agenda.paddingRadku,o.agenda.paddingRadku); setRange(refs.rowmin,s.agenda.minVyskaRadku,o.agenda.minVyskaRadku); setRange(refs.rowgap,s.agenda.mezeraRadku,o.agenda.mezeraRadku); setRange(refs.time,s.agenda.sirkaCasu,o.agenda.sirkaCasu); setRange(refs.sh,s.subnav.vyska,o.subnav.vyska); setRange(refs.sr,s.subnav.radius,o.subnav.radius); setRange(refs.sg,s.subnav.mezera,o.subnav.mezera); setRange(refs.sfont,s.subnav.pismo,o.subnav.pismo); setRange(refs.rfilter,s.pripominky.sirkaFiltru,o.pripominky.sirkaFiltru); setRange(refs.rheight,s.pripominky.vyskaRadku,o.pripominky.vyskaRadku); setRange(refs.rfont,s.pripominky.pismoRadku,o.pripominky.pismoRadku); setRange(refs.rtime,s.pripominky.pismoCasu,o.pripominky.pismoCasu); setRange(refs.rgroup,s.pripominky.pismoSkupiny,o.pripominky.pismoSkupiny); setRange(refs.rgap,s.pripominky.mezeraSkupin,o.pripominky.mezeraSkupin); setRange(refs.rradius,s.pripominky.radiusRadku,o.pripominky.radiusRadku); refs.columns.textContent=`Agenda: ${s.agenda.sloupce} ${s.agenda.sloupce===1?"sloupec":"sloupce"}`; }
  ...
  function udalosti(){refs.close.addEventListener("click",zavri);refs.min.addEventListener("click",()=>{minimal=!minimal;panel.classList.toggle("ln-nvt-minimized",minimal);refs.min.textContent=minimal?"+":"—";});refs.columns.addEventListener("click",()=>{const s=api()?.ziskejStav?.();api()?.nastav?.("agenda","sloupce",s?.agenda?.sloupce===2?1:2);});refs.copy.addEventListener("click",kopiruj);refs.reset.addEventListener("click",()=>api()?.obnovVychozi?.());refs.center.addEventListener("click",vycentruj);
    map(refs.screen,"kalendar","sirkaScreenu");map(refs.share,"kalendar","podilKalendare");map(refs.grid,"kalendar","sirkaMrizky");map(refs.dayh,"kalendar","vyskaDne");map(refs.daygap,"kalendar","mezeraDni");map(refs.dayrad,"kalendar","radiusDne");map(refs.dayfont,"kalendar","pismoDne");map(refs.apad,"agenda","padding");map(refs.arad,"agenda","radius");map(refs.afont,"agenda","pismo");map(refs.rowpad,"agenda","paddingRadku");map(refs.rowmin,"agenda","minVyskaRadku");map(refs.rowgap,"agenda","mezeraRadku");map(refs.time,"agenda","sirkaCasu");map(refs.sh,"subnav","vyska");map(refs.sr,"subnav","radius");map(refs.sg,"subnav","mezera");map(refs.sfont,"subnav","pismo");map(refs.rfilter,"pripominky","sirkaFiltru");map(refs.rheight,"pripominky","vyskaRadku");map(refs.rfont,"pripominky","pismoRadku");map(refs.rtime,"pripominky","pismoCasu");map(refs.rgroup,"pripominky","pismoSkupiny");map(refs.rgap,"pripominky","mezeraSkupin");map(refs.rradius,"pripominky","radiusRadku");refs.head.addEventListener("pointerdown",zacniDrag);window.addEventListener("pointermove",tahni);window.addEventListener("pointerup",konecDrag);window.addEventListener("pointercancel",konecDrag);window.addEventListener("lubanote:desktop-planner-visual-change",refresh);window.addEventListener("resize",omez);}

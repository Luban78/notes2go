...
(() => {
  ...
  function spustTodoSelection() {
    ...
    /* Core V2 je jediný runtime TODO/Bullet editoru. Diagnostika proto
       sleduje pouze jeho modelové řádky a gesture události. */
    window.LUBANOTE_V2_TODO_DRAG_VD_ACTIVE = true;

    if (!editor) {
      zapis("CHYBA | Core V2 editor není otevřený");
      window.LUBANOTE_V2_TODO_DRAG_VD_ACTIVE = false;
      return () => {};
    }

    const najdiV2Radek = target => target?.closest?.(
      ".ln-v2-odstavec.ln-v2-todo, .ln-v2-odstavec.ln-v2-bullet, .ln-v2-odstavec.ln-v2-ordered"
    ) || null;

    const zapisV2Udalost = (nazev, event) => {
      if (jeDebugPrvek(event.target)) return;
      const radek = najdiV2Radek(event.target);
      if (!radek && !(selectionMenu && selectionMenu.contains(event.target))) return;

      const bod = bodUdalosti(event);
      const casti = [
        nazev,
        `target=${popisPrvku(event.target)}`,
        `block=${radek?.dataset?.lnV2Blok || "-"}`,
        `typ=${radek?.classList?.contains("ln-v2-todo") ? "TODO" : radek?.classList?.contains("ln-v2-bullet") ? "BULLET" : radek?.classList?.contains("ln-v2-ordered") ? "ORDERED" : "-"}`
      ];
    ...
    [
      ...
      pridejPosluchac(uklidy, document, typ, event => zapisV2Udalost(typ, event), true);
    });
    ...
    /* Raw touch capture ukazuje přesně to, co Android WebView posílá
       Core V2 během longpress/MOVE. */
    ...
    zapis(`START TODO/BULLET CORE V2 | ${infoMenuVyberu()}`);
  ...
  function spustEditorSelection() {
    ...
    const zapisEditor = (typ, event = null, doplnek = "") => {
      ...
      const li = element?.closest?.(".ln-v2-radek") || null;
  ...
  function spustBulletDrag() {
    ...
    const popisRadku = (radek) => {
      if (!radek) return "row=NONE";
      return [
        `id=${radek.dataset.lnV2Blok || "-"}`,
        `typ=${radek.dataset.lnV2Typ || "-"}`,
        `uroven=${radek.dataset.lnV2Uroven || "0"}`,
        `selected=${radek.classList.contains("ln-v2-list-move-selected") ? "Y" : "N"}`
      ].join(" ");
    };

    const zapisUdalost = (typ, event, move = false) => {
      if (event && jeDebugPrvek(event.target)) return;
      if (move) {
        const ted = performance.now();
      ...
      const radek = element?.closest?.(".ln-v2-radek") || editor.querySelector(".ln-v2-list-move-selected") || null;
      const bod = event ? bodUdalosti(event) : null;
      zapis([
        typ,
        popisRadku(radek),
        bod ? `@${Math.round(bod.x)},${Math.round(bod.y)}` : "",
        `moveMode=${editor.classList.contains("ln-v2-list-drag-mode") ? "Y" : "N"}`
      ].filter(Boolean).join(" | "));
    };

    ["pointerdown", "pointerup", "pointercancel", "touchstart", "touchend", "touchcancel"].forEach((typ) => {
      pridejPosluchac(uklidy, editor, typ, (event) => zapisUdalost(typ, event), true);
    });
    ["pointermove", "touchmove"].forEach((typ) => {
      pridejPosluchac(uklidy, editor, typ, (event) => zapisUdalost(typ, event, true), true);
    });
    pridejPosluchac(uklidy, document, "lubanote:v2-todo-drag-debug", (event) => {
      zapis(`V2 LIST | ${event.detail?.text || ""}`);
    }, true);
    pridejPosluchac(uklidy, document, "lubanote:v2-stability-debug", (event) => {
      zapis(`V2 STABILITY | ${event.detail?.text || ""}`);
    }, true);

    zapis("START CORE V2 LIST DRAG");
    return () => uklidy.forEach((uklid) => uklid());
  }
  ...
  function nactiPerformanceBenchmark() {
    ...
    return new Promise((resolve, reject) => {
      const script = document.createElement("script");
      script.src = "performanceBenchmark.js?v=20260915-core-v2-hardcut-528";
      script.async = true;
